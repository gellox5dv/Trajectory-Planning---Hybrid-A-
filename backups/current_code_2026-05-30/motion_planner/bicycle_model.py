...
DEFAULT_COST_CONFIG_PATH = Path(__file__).parent / ".." / "configs" / "default_cost_config.yaml"
...
class Visualizer:
    ...
    def _draw_world(self, state: EgoStateStamped):
        ...
        ax.set_xlim(state.x - 15, state.x + 40)
        ax.set_ylim(-10, 10)
...
class Vehicle:
    ...
    def Print_ego(self):
        s = self.state
        phase_name = ['approach', 'pull out', 'pass', 'pull in', 'straighten', 'return', 'complete']
        print(
